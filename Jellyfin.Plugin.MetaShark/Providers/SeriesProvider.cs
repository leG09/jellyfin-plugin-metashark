...
namespace Jellyfin.Plugin.MetaShark.Providers
{
    public class SeriesProvider : BaseProvider, IRemoteMetadataProvider<Series, SeriesInfo>
    {
        ...
        public async Task<MetadataResult<Series>> GetMetadata(SeriesInfo info, CancellationToken cancellationToken)
        {
            ...
            {
                this.Log($"[Series TMDB ID提取] 开始检查文件名: {fileName}");
                var tmdbIdFromFileName = this.regTmdbIdAttribute.FirstMatchGroup(fileName);
                if (!string.IsNullOrWhiteSpace(tmdbIdFromFileName))
                {
                    this.Log($"[Series TMDB ID提取] ✓ 成功从文件名提取到TMDB ID: {tmdbIdFromFileName} (文件名: {fileName})");
                    tmdbId = tmdbIdFromFileName;
                }
                else
                {
                    this.Log($"[Series TMDB ID提取] ✗ 文件名中未找到TMDB ID格式 (文件名: {fileName})");
                }
            }
            else
            {
                this.Log($"[Series TMDB ID提取] 已存在TMDB ID: {tmdbId}，跳过文件名提取");
            }
